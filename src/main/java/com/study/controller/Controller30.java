...
public class Controller30 {
    ...
    private String update1(MyBean254Customer customer, RedirectAttributes rttr) throws SQLException {
        String sql = """
                UPDATE Customers
                SET CustomerName = ?,
                    ContactName = ?,
    ...
    private String update2(MyBean258Employee employee, RedirectAttributes rttr) throws SQLException {
        String sql = """
                UPDATE Employees
                SET LastName = ?,
                    FirstName = ?,
        ...
        try (pstmt; conn) {
            ...
            int rowCount = pstmt.executeUpdate();
            if (rowCount > 0) {
                rttr.addFlashAttribute("message", employee.getId() + "번 직원이 수정되었습니다.");
            } else {
                rttr.addFlashAttribute("message", "수정되지 않았습니다.");
            }
        }
